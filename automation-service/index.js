const { ethers } = require('ethers');
const cron = require('node-cron');
require('dotenv').config();

// Configuration
const RPC_URL = process.env.RPC_URL || 'http://localhost:8545';
const PRIVATE_KEY = process.env.PRIVATE_KEY;
const CONTRACT_ADDRESS = process.env.CONTRACT_ADDRESS;

// Oracle addresses for different assets
const BITCOIN_ORACLE_ADDRESS = process.env.BITCOIN_ORACLE_ADDRESS;
const ETH_ORACLE_ADDRESS = process.env.ETH_ORACLE_ADDRESS;
const GOLD_ORACLE_ADDRESS = process.env.GOLD_ORACLE_ADDRESS;

// Market configurations
const DAILY_MARKETS = [
  { name: 'Bitcoin', oracle: BITCOIN_ORACLE_ADDRESS },
  { name: 'Ethereum', oracle: ETH_ORACLE_ADDRESS },
  { name: 'Gold', oracle: GOLD_ORACLE_ADDRESS },
];

// Contract ABI - only the functions we need
const CONTRACT_ABI = [
  'function createDailyOverUnder(address _oracle, uint256 _endTime, string _assetName) external',
  'function resolveMarket(uint256 _marketId) external',
  'function getMarketCount() external view returns (uint256)',
  'function getMarket(uint256 _marketId) external view returns (tuple(string name, string outcomeA, string outcomeB, address oracleA, address oracleB, int256 targetPrice, uint256 totalPoolA, uint256 totalPoolB, uint256 endTime, bool resolved, bool outcomeAWon, uint256 fee, uint8 marketType, int256 creationPrice, bool isAutomatic))',
];

// Oracle ABI - for updating price
const ORACLE_ABI = [
  'function updateAnswer(int256 _answer) external',
  'function latestRoundData() external view returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)',
];

let provider;
let wallet;
let contract;
let isProcessing = false; // Lock to prevent concurrent executions

// Initialize connection
async function initialize() {
  console.log('Initializing automation service...');

  provider = new ethers.JsonRpcProvider(RPC_URL);
  wallet = new ethers.Wallet(PRIVATE_KEY, provider);
  contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, wallet);

  console.log('Connected to RPC:', RPC_URL);
  console.log('Automation service address:', wallet.address);
  console.log('Contract address:', CONTRACT_ADDRESS);
  console.log('');
  console.log('Managing daily markets for:');
  DAILY_MARKETS.forEach(market => {
    if (market.oracle) {
      console.log(`  - ${market.name}: ${market.oracle}`);
    }
  });
  console.log('');
}

// Load timing configuration based on environment
// Use test-config.js for local testing (5 min markets)
// Use production-config.js for testnet/mainnet (24 hour markets)
const configFile = process.env.USE_PRODUCTION_CONFIG === 'true'
  ? './production-config.js'
  : './test-config.js';

const { getNextMidnight, getOneMinuteAfterMidnight } = require(configFile);

console.log(`Using config: ${configFile}`);

// Check if there's an active daily market for a specific oracle
async function getActiveDailyMarket(oracleAddress) {
  try {
    const marketCount = await contract.getMarketCount();

    for (let i = marketCount - 1n; i >= 0n; i--) {
      const market = await contract.getMarket(i);

      // Check if it's a DAILY_OVER_UNDER market (marketType = 1)
      // And matches the oracle address
      if (Number(market.marketType) === 1 &&
          market.isAutomatic &&
          !market.resolved &&
          market.oracleA.toLowerCase() === oracleAddress.toLowerCase()) {
        return {
          id: i,
          ...market
        };
      }
    }

    return null;
  } catch (error) {
    console.error(`Error getting active daily market for ${oracleAddress}:`, error.message);
    return null;
  }
}

// Create a new daily over/under market for a specific asset
async function createDailyMarket(assetName, oracleAddress) {
  try {
    console.log(`Creating new daily ${assetName} over/under market...`);

    // Get current price
    const oracleContract = new ethers.Contract(oracleAddress, ORACLE_ABI, wallet);
    const roundData = await oracleContract.latestRoundData();
    const currentPrice = roundData.answer;
    console.log(`Current ${assetName} price:`, ethers.formatUnits(currentPrice, 8));

    // Only update mock oracle price if using local/test environment
    // On testnet/mainnet, we use real Chainlink oracles that we cannot update
    const isLocalEnvironment = RPC_URL.includes('localhost') || RPC_URL.includes('127.0.0.1') || RPC_URL.includes('8545');

    if (isLocalEnvironment) {
      // Random change with 50/50 chance of increment or decrement (41-341)
      const randomAmount = Math.floor(Math.random() * 301) + 41; // Random between 41 and 341
      const isIncrement = Math.random() >= 0.5; // 50/50 chance
      const priceChange = isIncrement ? randomAmount : -randomAmount;
      const newPrice = currentPrice + BigInt(priceChange * 1e8); // Convert to 8 decimals

      console.log(`Updating mock oracle price by ${isIncrement ? '+' : ''}$${priceChange} to $${ethers.formatUnits(newPrice, 8)}`);
      const updateTx = await oracleContract.updateAnswer(newPrice);
      const updateReceipt = await updateTx.wait();
      console.log('Price updated! Tx:', updateReceipt.hash);

      // Small delay to ensure nonce is updated
      await new Promise(resolve => setTimeout(resolve, 500));
    } else {
      console.log('Using real Chainlink oracle - price will update automatically');
    }

    const endTime = getNextMidnight();
    console.log('Market will end at:', new Date(endTime * 1000).toLocaleString());

    const tx = await contract.createDailyOverUnder(
      oracleAddress,
      endTime,
      assetName
    );

    console.log('Transaction sent:', tx.hash);
    const receipt = await tx.wait();
    console.log(`${assetName} market created! Gas used:`, receipt.gasUsed.toString());
    console.log('');

    return true;
  } catch (error) {
    console.error(`Error creating ${assetName} daily market:`, error.message);
    return false;
  }
}

// Resolve the daily market
async function resolveDailyMarket(marketId) {
  try {
    console.log(`Resolving daily market #${marketId}...`);

    const tx = await contract.resolveMarket(marketId);
    console.log('Transaction sent:', tx.hash);

    const receipt = await tx.wait();
    console.log('Market resolved! Gas used:', receipt.gasUsed.toString());
    console.log('');

    return true;
  } catch (error) {
    console.error('Error resolving market:', error.message);
    return false;
  }
}

// Main job that runs every minute
async function checkAndExecute() {
  // Prevent concurrent executions
  if (isProcessing) {
    console.log('Already processing, skipping this check...');
    return;
  }

  isProcessing = true;
  const now = Math.floor(Date.now() / 1000);
  const currentTime = new Date().toLocaleTimeString();

  console.log(`[${currentTime}] Checking for tasks...`);

  try {
    // Check each configured daily market
    for (const marketConfig of DAILY_MARKETS) {
      if (!marketConfig.oracle) {
        console.log(`Skipping ${marketConfig.name} - no oracle configured`);
        continue;
      }

      console.log(`\nChecking ${marketConfig.name} market...`);
      const activeDailyMarket = await getActiveDailyMarket(marketConfig.oracle);

      if (activeDailyMarket) {
        // Access using array indices since contract returns a tuple
        const marketEndTime = Number(activeDailyMarket[8]); // endTime is index 8
        const resolved = activeDailyMarket[9]; // resolved is index 9

        console.log(`Active ${marketConfig.name} market found (ID: ${activeDailyMarket.id})`);
        console.log(`End time: ${new Date(marketEndTime * 1000).toLocaleString()}`);
        console.log(`Resolved: ${resolved}`);
        console.log(`Time until end: ${Math.max(0, marketEndTime - now)}s`);

        // Check if it's time to resolve (at or after end time)
        if (!resolved && now >= marketEndTime) {
          console.log(`Time to resolve the ${marketConfig.name} market!`);
          const success = await resolveDailyMarket(activeDailyMarket.id);

          if (success) {
            // Wait 10 seconds then create new market
            console.log('Waiting 10 seconds before creating new market...');
            await new Promise(resolve => setTimeout(resolve, 10000));
            await createDailyMarket(marketConfig.name, marketConfig.oracle);
          }
        } else if (resolved) {
          // Market is resolved but new one hasn't been created yet
          const tenSecondsAfter = marketEndTime + 10;
          if (now >= tenSecondsAfter) {
            console.log(`Creating new ${marketConfig.name} market (10 seconds after resolution)...`);
            await createDailyMarket(marketConfig.name, marketConfig.oracle);
          }
        } else {
          // Market is active and not yet ready to resolve
          console.log(`${marketConfig.name} market is active. Waiting for end time...`);
        }
      } else {
        // No active daily market - create one
        console.log(`No active ${marketConfig.name} market found. Creating one...`);
        await createDailyMarket(marketConfig.name, marketConfig.oracle);

        // Add delay between market creations to avoid nonce issues
        await new Promise(resolve => setTimeout(resolve, 2000));
      }
    }

  } catch (error) {
    console.error('Error in checkAndExecute:', error.message);
  }

  isProcessing = false;
  console.log('---');
}

// Start the service
async function start() {
  await initialize();

  console.log('Starting automation service...');
  console.log('Checking every minute for markets to resolve/create');
  console.log('Press Ctrl+C to stop');
  console.log('========================');
  console.log('');

  // Run immediately on start
  await checkAndExecute();

  // Then run every minute
  cron.schedule('* * * * *', async () => {
    await checkAndExecute();
  });
}

// Handle shutdown
process.on('SIGINT', () => {
  console.log('\nShutting down automation service...');
  process.exit(0);
});

// Start the service
start().catch(console.error);
